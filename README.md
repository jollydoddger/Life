...
**GPS runs when you are tracking, and not otherwise.** Recording a walk
holds GPS for as long as the walk lasts, so glances during a walk are
instant. Outside a walk the watch waits for a fix like any other app — the
first look after the screen has slept costs about twenty seconds of grey
arrow, which is the honest price of not holding the receiver open all day.

There is a switch in the phone's Settings that keeps the fix warm for 90
minutes after each look, and it is **off by default**: every glance and every
touch pushes that deadline out again, so in ordinary use it never expires and
the watch holds GPS permanently. That was the default for exactly one day
before being reported, correctly, as always-on.
